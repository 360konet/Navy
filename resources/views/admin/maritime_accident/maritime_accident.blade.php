...
<div class="card-body h-full flex flex-col justify-between">
    ...
    <div>
        <h1 class="text-lg font-bold tracking-wide">Maritime Accidents</h1>
    ...
    <div class="flex flex-row mt-10 items-end">
        ...
        <div class="flex-1">
            ...
            <a href="{{url('/maritime-accident-form')}}" class="btn-shadow py-3">
                Add Record
            </a>
        </div>
    ...
    <table id="yourTableId" class="table-auto w-full mt-5 text-right">
        ...
        <thead>
            <tr>
                ...
                <td class="py-4 font-extrabold text-sm">Accident</td>
                <td class="py-4 font-extrabold text-sm">Date</td>
        ...
        @foreach($maritime as $maritime)
            <!-- item -->
            <tr class="">
                <td class="py-4 text-sm text-gray-600 flex flex-row items-center">Latitude: ({{$maritime->latitude_degrees}}<sup>0</sup> {{$maritime->latitude_minutes}}<sup>1</sup> {{$maritime->latitude_cardinal}}), Longitude:({{$maritime->longitude_degrees}}<sup>0</sup> {{$maritime->longitude_minutes}}<sup>1</sup> {{$maritime->longitude_cardinal}})</td>
                <td class="py-4 text-xs text-gray-600">{{$maritime->accident_category}}, {{$maritime->other_accident}}</td>
                <td class="py-4 text-xs text-gray-600">{{$maritime->date}}</td>
                <td class="py-4 text-xs text-gray-600"> <div class="dropdown">
                        <button class="btn-gray">More Info</button>
                        <div class="dropdown-content">
                            <a href="{{url('/edit-maritime-accident-form-'.$maritime->id)}}">Edit</a>
                            <a href="{{url('/view-maritime-accident-'.$maritime->id)}}">View</a>
                            <a href="#" onclick="confirmDelete('{{url('/delete-maritime-accident-'.$maritime->id)}}')">Delete</a>
